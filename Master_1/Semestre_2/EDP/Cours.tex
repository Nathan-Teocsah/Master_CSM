...
\begin{document}
	...
	\underline{\textbf{Reformulation :}} \begin{minipage}[t]{15cm}
		...
		est, par exemple, $f^0(\alpha) = \mathbb{1}_\Omega(\alpha)$ pour un certain ouvert $\Omega \subset \R^{6N}$. \hspace{1cm} \begin{minipage}[t]{5cm}
	...
	\textbf{\underline{Conclusion :}} \begin{minipage}[t]{15cm}
		$\forall t, x_0$
		\[\begin{aligned}
			\dfrac{\partial f}{\partial t}(t,x) + g(x)\dfrac{\partial f}{\partial x}(t,x) 
			&= \left(\dfrac{\partial f}{\partial t} + g\dfrac{\partial f}{\partial x}\right)(t,\underbrace{X(t,0,\overbrace{X(0,t,x)}^{\overset{(det)}{=x_0}})}_{\text{(rel de groupe pour le flot)} })\\
			&=0
		\end{aligned}\]
	\end{minipage}
	...
	\textbf{\underline{Exemple :}} \fbox{$g(\alpha) = 1$} $\begin{cases}
		\text{EDO : } \begin{cases}
			x'(t) = 1\\
			x(0) = x_0
	...
	\hspace{-.5cm}\includegraphics[scale=.35]{photo8.png}\hspace{.2cm}\includegraphics[scale=.45]{photo9.png}\hspace{.2cm}\includegraphics[scale=.5]{photo10.png}
