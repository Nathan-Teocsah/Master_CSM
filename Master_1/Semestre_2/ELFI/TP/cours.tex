...
\section{TP séance 2}
	...
	\begin{enumerate}[label=$\star$]
		...
		\item $\hat\omega_i$, $i=1,\cdots,N^K$ sur $\widehat{K}$, \rouge{et leurs dérivées premières.}
		
		\item $F_K$ définie à partir des $\hat\omega_i$, $F_K(\hat x) = \displaystyle\sum_{i=1}^{N^K}\hat\omega_i(\hat x)q_i^K$.\\
