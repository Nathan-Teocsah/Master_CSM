...
\begin{document}
	...
	\[
	\left(\overset\sim{\mathbb{D}}_{RL}\frac1\lambda + (\overset\sim{\mathbb{D}}_R+\overset\sim{\mathbb{D}}_R)+\overset\sim{\mathbb{D}}_{LR}\right)\emptyset = 0 \hspace{1cm} \rouge{!! \hspace{.2cm}\overset\sim{\mathbb{D}} \not=\overset\sim{\mathbb{K}}-\omega^2\overset\sim{\mathbb{M}}}
	\]
...
\end{rmq}
	...
	\begin{ex} \begin{minipage}[t]{15cm}
	...
	\begin{sol}
		...
		\begin{enumerate}
			...
			\begin{minipage}[t]{16cm}
				L'équation de dispersion (en $k(\omega)$ uniquement car nous avons effectué une condensation dynamique) est alors : $\left(\vert{D_{RL}}\frac1\lambda + (D_L+\rouge{D_R})+\bleu{D_{LR}}\lambda\right)\emptyset = 0$
			\end{minipage}
	...
	\begin{sol}$ $\\
		...
		\end{bmatrix} = \begin{bNiceMatrix}[margin=1em]
		m_0 & & 0 & 0 \\\\
		0 & & m_1 & 0 \\
		0 & & 0 & m_2
		\CodeAfter
