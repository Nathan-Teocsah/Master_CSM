...
def heun(a0, alpha, T, dt) :
    ...
    for i in range(1, len(t)) :
        k1 = da_dt(a[i-1], alpha)
        k2 = da_dt(a[i-1] + k1 * dt, alpha)
        a[i] = a[i-1] + (k1 + k2) * dt / 2 # Méthode de Heun
    return t, a
...
Alpha = alpha[index]
dt = 10**3 # pas de temps en années
dt = dt * 365.25 * 24 * 3600 # conversion du pas de temps en secondes
print(f"Pas de temps : {dt:E} secondes.")
print(f"Pas de temps : {dt/(3600*24*365.25):.0E} années.")
print(f"Nombre de points : {int(T/dt):.2E}.")
...
print(f"Phobos atteint la Roche après {t_ref[zero_ref]/(365.25*24*3600e6):.6f} millions d'années (référence RK45).")
print(f"----> Différence entre les deux méthodes : {(t[zero] - t_ref[zero_ref])/(365.25*24*3600*10**6):.6f} millions d'années.")
print(f"sup |met_Heun - met_RK45| = {np.max(np.abs(a - a_ref)):.6f} mètres.")




plt.figure()
plt.plot(t_ref/(365.25*24*3600 * 10**6), a_ref*10**(-3), label="Chute de a (km) pour alpha = 0.2")
...
plt.figure()
plt.plot(t_ref/(365.25*24*3600 * 10**6), Delta_t(a_ref, Alpha)/60, label="Lag temporel (en min) pour alpha = 0.2")
...
dt0 = 10**2  # pas de temps initial pour l'erreur (en secondes) : 100 ans
dt_fin = 10**5
...
for i in range(Nb_point) :
    ...
    print(f"\rprogression : {(i+1)/Nb_point*100:.2f} % pour dt = {dt/(365.25*24*3600):.2E} années.", end='', file=sys.stdout)
    temps[i] = time.time()
    t,a = heun(a0, Alpha, T, dt)
