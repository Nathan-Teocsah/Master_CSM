...
Erreur_heun = np.zeros(Nb_point) # tableau pour stocker les erreurs
...
for i in range(Nb_point) :
    ...
    t,a = heun(a0, Alpha, T, dt)
    temps_heun[i] = time.time() - temps_heun[i]
    ind_min = np.min([Sup(a),Sup(sol_ref.sol(t)[0])])

    Erreur_heun[i] = np.max(np.abs(a[0:ind_min] - sol_ref.sol(t)[0][0:ind_min]))

    Erreur_rel_heun[i] = Erreur_heun[i]/np.max(np.abs(sol_ref.sol(t)[0][0:ind_min]))
...
print("coef_max = ",np.max(comparaison))
print("coef_moyen = ",np.mean(comparaison))
