import numpy as np
import matplotlib.pyplot as plt
import sys
from scipy.integrate import solve_ivp
import time
from sklearn.linear_model import LinearRegression

'''******************************************
                CONSTANTES
******************************************'''

G = 6.67430e-11 # constante gravitationnelle
M0 = 6.4185e23 # masse de Mars
m = 1.06e16 # masse de Phobos
R = 3396.2e3 # rayon de Mars (en mètres)
k2 = 0.169 # 0.169 nombre de Love de Mars : https://arxiv.org/html/2405.05519v1
omega_p = 2*np.pi/(24.622962*3600) # vitesse de rotation de Mars (rad/s)
a0 = 9377e3 # demi-grand axe phobos (en mètres)
lim_roche = 2.2*R# distance de Roche pour Phobos (en mètres) : https://www.insu.cnrs.fr/fr/cnrsinfo/phobos-la-lune-condamnee-pourquoi-mars-va-eroder-puis-disloquer-son-satellite
a_min = lim_roche

'''*******************************************
                    FONCTIONS
 ******************************************'''

def E(alpha) : # sec/rad (Q = E^alpha X^alpha)
    match alpha :
        case 0.2 : return  1201*10**5*24*3600
        case 0.3 : return 81028*24*3600
        case 0.4 : return 2104*24*3600
        case 0 : return 2*Q

def n(a) : # fréquence orbitale de Phobos (rad/s)  
    return np.sqrt(G*(M0+m)/a**3)

def X(a) : # fréquence de marée principale (s^(-1))
    return 2*np.abs(omega_p - n(a))

def Delta_t(a, alpha) : # lag temporel (s)
    match alpha :
        case 0 : return (X(a)*Q)**(-1)
        case _ : return E(alpha)**(-alpha) * X(a)**(-(alpha+1))

def da_dt(a, alpha) : # dérivée du demi-grand axe (m/s)
    numerateur = 6* k2 * R**5 * n(a) * m * Delta_t(a, alpha)
    denominateur = M0 * a**4
    return -numerateur/denominateur * (n(a) - omega_p)

def da_dt_Kaula(a) : # dérivée du demi-grand axe (m/s)
    numerateur = 3* k2 * R**5 * G * m
    denominateur = Q * np.sqrt(G*(M0+m)) * a**(5.5)
    return -numerateur/denominateur

def euler_explicite(a0, alpha, T, dt,a_min) :
    a = [a0]
    t = [0]
    while t[-1] < T :
        if a[-1] < a_min :
            break
        a.append(a[-1] + da_dt(a[-1], alpha) * dt) # Euler explicite
        t.append(t[-1]+dt)
    return np.array(t), np.array(a)

def heun(a0, alpha, T, dt,a_min) :
    a = [a0]
    t = [0]
    while t[-1] < T :
        if a[-1] < a_min :
            break
        k1 = da_dt(a[-1], alpha)
        k2 = da_dt(a[-1] + k1 * dt, alpha)
        a.append(a[-1] + (k1 + k2) * dt / 2) # Euler explicite
        t.append(t[-1]+dt)
    return np.array(t), np.array(a)


'''********************************************************
                CHOIX DES CONSTANTES
********************************************************'''

alpha = float(input("\n1. Choisir une valeur de alpha (0.2, 0.3 ou 0.4) : "))
T = float(input("2. Choisir un temps (en Ma) de simulation : "))
reponse = input("3. Voulez vous lancer la simulation jusqu'à ce que Phobos atteigne :\n  (M) Mars\n  (R) Limite de Roche\nChoix : ")
a_min = lim_roche
if reponse=="M" :
    a_min = R
T = T * 1e6 * 365.25 * 24 * 3600 # conversion millions d'années en secondes


#----------------------- Création fonction de référence --------------------------

def da_dt_for_solve_ivp(t, a):
    return [da_dt(a, alpha)]  

sol_ref = solve_ivp(
    da_dt_for_solve_ivp,
    t_span=(0, T),
    y0=[a0],
    method='RK45',
    rtol=1e-12,
    atol=1e-6, 
    dense_output=True
)

'''***********************************************************
                    Calcul de l'erreur
**********************************************************'''


print("\n----- Calcul de l'erreur pour différents pas -----")
Nb_point = 50 # nombre de points pour l'erreur
dt0 = 10**2  # pas de temps initial pour l'erreur (en années)
dt_fin = 10**4

dt_fin = dt_fin * 365.25 * 24 * 3600 # conversion du pas de temps final en secondes
dt0 = dt0 * 365.25 * 24 * 3600 # conversion du pas de temps initial en secondes
DT = np.linspace(dt0, dt_fin, Nb_point)  # pas de temps (en années) pour l'erreur

def Sup(a) :
    ind = 0
    for i in range(len(a)) :
        if a[i] >= a_min :
            ind = ind + 1
        else :
            break
    return ind

Erreur_rel = np.zeros(Nb_point) # tableau pour stocker les erreurs

Erreur_rel_heun = np.zeros(Nb_point) # tableau pour stocker les erreurs

temps = np.zeros(Nb_point)
temps_heun = np.zeros(Nb_point)

temps_total=time.time()
for i in range(Nb_point) :
    dt = DT[i]
    print(f"\rprogression : {i/Nb_point*100:.2f} % -- dt = {dt/(3600*24*365.25):.0f} ans", end='', file=sys.stdout)

    #======== Euler Explicite =====
    temps[i] = time.time()
    t,a = euler_explicite(a0, alpha, T, dt, a_min)
    temps[i] = time.time() - temps[i]
    ind_min = np.min([Sup(a),Sup(sol_ref.sol(t)[0])])
    Erreur_rel[i] = np.max(np.abs(a[0:ind_min] - sol_ref.sol(t)[0][0:ind_min]))
    Erreur_rel[i] = Erreur_rel[i]/np.max(np.abs(sol_ref.sol(t)[0][0:ind_min]))

    #======== Heun =====
    temps_heun[i] = time.time()
    t,a = heun(a0, alpha, T, dt,a_min)
    temps_heun[i] = time.time() - temps_heun[i]
    ind_min = np.min([Sup(a),Sup(sol_ref.sol(t)[0])])
    Erreur_rel_heun[i] = np.max(np.abs(a[0:ind_min] - sol_ref.sol(t)[0][0:ind_min]))
    Erreur_rel_heun[i] = Erreur_rel_heun[i]/np.max(np.abs(sol_ref.sol(t)[0][0:ind_min]))
    

print(f"\nTemps total = {time.time()-temps_total:.2f} s.")


#----------------------- Calcul coef erreur relative ---------------------------

# Sample data
x = np.log(DT).reshape(-1, 1)  # Reshape for scikit-learn
y = np.log(Erreur_rel)

# Create model instance
model = LinearRegression()

# Fit the model
model.fit(x, y)

# Get model parameters
print("\n---------- Erreur relative - Euler Explicite -----------")
print(f"Estimation de Ordre et C : |a_n-a(t_n)|/sup|a| <= C * h^Ordre")
print(f"---> Ordre = {model.coef_[0]:.2E}")
print(f"---> C = {np.exp(model.intercept_):.2E}")

#-------------------- Calcul Erreur_rel Heun ---------------

# Sample data
x_rel = np.log(DT).reshape(-1, 1)  # Reshape for scikit-learn
y_rel = np.log(Erreur_rel_heun)

# Fit the model
model.fit(x_rel, y_rel)

# Get model parameters
print("\n---------- Erreur relative - Heun -----------")
print(f"Estimation de Ordre et C : |a_n-a(t_n)|/sup|a| <= C * h^Ordre")
print(f"---> Ordre = {model.coef_[0]:.2E}")
print(f"---> C = {np.exp(model.intercept_):.2E}")


#comparaison

comparaison = temps_heun/temps

print("\n-------------------------------------------------")
print(f"Euler est en moyenne  = {np.mean(comparaison):.1f} fois plus rapide que Heun")

plt.figure()
plt.plot(DT/(365.25*24*3600), comparaison)
plt.xlabel("pas de temps (années)")
plt.ylabel("comparaison")
plt.title("Heun est combien de fois plus lent que Euler ?")
plt.xscale("log")
plt.yscale("log")
plt.grid()


#----------------- GRAPHIQUE -----------------------------------

plt.figure()
plt.plot(DT/(365.25*24*3600), Erreur_rel,label="Euler")
plt.plot(DT/(365.25*24*3600), Erreur_rel_heun,label="Heun")
plt.xlabel("pas de temps (années)")
plt.ylabel("Erreur relative")
plt.title("Comparaison de l'erreur relative")
plt.xscale("log")
plt.yscale("log")
plt.grid()
plt.legend()

plt.figure()
plt.plot(DT/(365.25*24*3600), temps, label="Euler")
plt.plot(DT/(365.25*24*3600), temps_heun, label="Heun")
plt.xlabel("pas de temps (années)")
plt.ylabel("Temps (s.)")
plt.title("Comparaison du temps")
plt.xscale("log")
plt.yscale("log")
plt.grid()
plt.legend()

plt.show()
