...
\begin{document}
	...
	\begin{frame}{Prérequis}
		...
		\begin{center}
			...
			\only<8>{
				\vspace{1cm}
				\underline{Matrice $M_i \in \mathcal M_{2,2}(\mathbb{R})$ associée à la cellule $C_i$ :}
	...
	\begin{frame}{Majoration de l'erreur}
		...
		Majorations des erreurs pour \only<3-4>{$h = \max_i \max_{j \in \mathcal N_i} d(x_i,x_j)$}\only<5>{$\textcolor{red}{h = \max_i \max_{j \in \mathcal N_i} d(x_i,x_j)}$}\only<6>{$\textcolor{blue}{h = \max_i \max_{j \in \mathcal N_i} \mathrm{diam}(C_i)+\mathrm{diam}(C_j)}$}\only<7-8>{$h = \max_i \max_{j \in \mathcal N_i} \mathrm{diam}(C_i)+\mathrm{diam}(C_j)$}\only<9>{$\textcolor{red}{h = \max_i \max_{j \in \mathcal N_i} \mathrm{diam}(C_i)+\mathrm{diam}(C_j)}$}\only<10>{$\textcolor{blue}{h = \dfrac{\left(\max_i \max_{j \in \mathcal N_i} \mathrm{diam}(C_i)+\mathrm{diam}(C_j)\right)^3}{\min_i \abs{C_i}}}$}\only<11->{$h = \dfrac{\left(\max_i \max_{j \in \mathcal N_i} \mathrm{diam}(C_i)+\mathrm{diam}(C_j)\right)^3}{\min_i \abs{C_i}}$} :
		...
		\only<8>{
			\vspace{1cm}
		\begin{equation}
			|\!| {R^{(i)}}|\!| \leq 2HN_{max}\frac{h^3}{\abs{C_i}}
		\end{equation}
		}
		\only<9>{
			\vspace{1cm}
			\begin{equation}
				\textcolor{red}{|\!| {R^{(i)}}|\!| \leq 2{|\!|\nabla^2 u|\!|_\infty}N_{max}\frac{h^3}{\abs{C_i}}}
			\end{equation}
		}
		\only<10>{
			\vspace{1cm}
	...
	\begin{frame}{Majoration de $|\!|\nabla u_i - \overline{\nabla u(x_i)}|\!|_2$ : maillage triangulaire}
		...
		$\implies$ Maille de Delaunay : $\mu_{\min} \geq 26.5^\circ$\pause $\implies$ $|\!|\nabla u_i - \overline{\nabla u(x_i)}|\!| \leq 40.8 {|\!|\nabla^2 u|\!|_\infty}\, h$.
	\end{frame}
	
	
	\begin{frame}{Tests numérique : Fonctions tests}
		\begin{itemize}
			\only<1,2,4>{\item Gaussienne : $\mathrm{gauss}(x,y) = \exp\left(\dfrac{(x-x_c)^2+(y-y_c)^2}{2\sigma^2}\right)$\\
			\only<1>{
				\vspace{.5cm}
				\centering
			...
			\only<2-4>{
			\item Fonction anisotrope et fortement oscillante :
			
			\vspace{.2cm}
			
			 $u_1(x,y) = \left(\sin(\pi x)\cos(3\pi x + 0.5 \pi) + \sin(xy\pi) + \pi\sin(2\pi xy)\right)\mathrm {gauss}(x,y)$\\
			 \only<3>{
			 	\vspace{.5cm}
			 	\centering
			 	\includegraphics[scale=.2]{image/ani.png}
			 	\captionof{figure}{Fonction anisotrope.}
			 }
			 
			 \vspace{.5cm}}
			 
			 \only<4->{\item Fonction plateau :
	...
	\begin{frame}{Tests numérique : Maillages}
		\begin{figure}[h]
	...
	\begin{frame}{Résultats : gaussienne}
		...
		\begin{figure}
			\begin{subfigure}{.4\textwidth}
				\centering
				\includegraphics[scale=.3]{image/carre.png}
				\caption{$\varphi = 0$.}
			\end{subfigure}
			\begin{subfigure}{.4\textwidth}
				\centering
				\includegraphics[scale=.3]{image/deforme_05.png}
				\caption{$\varphi = 0.5$.}
			\end{subfigure}
			\begin{subfigure}{.4\textwidth}
				\centering
				\includegraphics[scale=.3]{image/deforme_09.png}
				\caption{$\varphi = 0.9$.}
			\end{subfigure}
			\begin{subfigure}{.4\textwidth}
				\centering
				\includegraphics[scale=.3]{image/ffm.png}
				\caption{Delaunay.}
			\end{subfigure}
		\end{figure}}
		...
		\begin{NiceTabular}{cccc}
			Maillage & $\mathrm L^1$ & $\mathrm L^2$ & $\mathrm L^\infty$ \\
			\hline
			$\varphi=0$ & 1.917 & 1.855 & 1.744 \\
			$\varphi=0.5$ & 1.029 & 0.951 & 0.632 \\
			$\varphi=0.9$ & 1.001 & 0.891 & 0.364 \\
			Delaunay & 1.144 & 1.149 & 0.996
		\end{NiceTabular}
		}
	\end{frame}
	
	
	\begin{frame}[fragile]{Résultats : fonction anisotrope}
		\only<1>{
			\begin{figure}
				\centering
				\includegraphics[scale=.6]{image/carre_ani.png}
				\caption{Maillage cartésien ($\varphi = 0$).}
		\end{figure}}
		...
		\only<2>{
			\begin{figure}
				\centering
				\includegraphics[scale=.6]{image/deforme_05_ani.png}
				\caption{Maillage cartésien déformé de déformation $\varphi = 0.5$.}
		\end{figure}}
		...
		\only<3>{
			\begin{figure}
				\centering
				\includegraphics[scale=.6]{image/deforme_09_ani.png}
				\caption{Maillage cartésien déformé de déformation $\varphi = 0.9$.}
		\end{figure}}
		...
		\only<4>{
			\begin{figure}
				\centering
				\includegraphics[scale=.6]{image/ffm_ani.png}
				\caption{Maillage de Delaunay.}
		\end{figure}}
		...
		\only<5>{
			...
			\begin{figure}
				\begin{subfigure}{.4\textwidth}
					\centering
					\includegraphics[scale=.3]{image/carre_ani.png}
					\caption{$\varphi = 0$.}
				\end{subfigure}
				\begin{subfigure}{.4\textwidth}
					\centering
					\includegraphics[scale=.3]{image/deforme_05_ani.png}
					\caption{$\varphi = 0.5$.}
				\end{subfigure}
				\begin{subfigure}{.4\textwidth}
					\centering
					\includegraphics[scale=.3]{image/deforme_09_ani.png}
					\caption{$\varphi = 0.9$.}
				\end{subfigure}
				\begin{subfigure}{.4\textwidth}
					\centering
					\includegraphics[scale=.3]{image/ffm_ani.png}
					\caption{Delaunay.}
				\end{subfigure}
		\end{figure}}
		...
		\only<6>{
			...
			\begin{center}
				\begin{NiceTabular}{cccc}
					Maillage & $\mathrm L^1$ & $\mathrm L^2$ & $\mathrm L^\infty$ \\
					\hline
					$\varphi=0$ & 1.600 & 1.621 & 1.636 \\
					$\varphi=0.5$ & 1.175 & 1.188 & 1.376 \\
					$\varphi=0.9$ & 1.105 & 1.115 & 0.101 \\
					Delaunay & - & - & -
				\end{NiceTabular}
			\end{center}
	...
	\begin{frame}{Résultats : fonction plateau}
		...
		\only<5>{
			...
			\begin{figure}
				\begin{subfigure}{.4\textwidth}
					\centering
					\includegraphics[scale=.3]{image/carre_plateau.png}
					\caption{$\varphi = 0$.}
				\end{subfigure}
				\begin{subfigure}{.4\textwidth}
					\centering
					\includegraphics[scale=.3]{image/deforme_05_plateau.png}
					\caption{$\varphi = 0.5$.}
				\end{subfigure}
				\begin{subfigure}{.4\textwidth}
					\centering
					\includegraphics[scale=.3]{image/deforme_09_plateau.png}
					\caption{$\varphi = 0.9$.}
				\end{subfigure}
				\begin{subfigure}{.4\textwidth}
					\centering
					\includegraphics[scale=.3]{image/ffm_plateau.png}
					\caption{Delaunay.}
				\end{subfigure}
		\end{figure}}
		...
		\begin{NiceTabular}{cccc}
			Maillage & $\mathrm L^1$ & $\mathrm L^2$ & $\mathrm L^\infty$ \\
			\hline
			$\varphi=0$ & 1.694 & 1.426 & 0.957 \\
			$\varphi=0.5$ & 1.116 & 1.282 & 0.723 \\
			$\varphi=0.9$ & 0.739 & - & - \\
			Delaunay & 1.041 &0.792 & 0.274
		\end{NiceTabular}
	}
	\end{frame}
