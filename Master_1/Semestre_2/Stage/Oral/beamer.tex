...
\begin{document}
	...
	\begin{frame}{Introduction}
		\begin{itemize}
			\item Maillages non-structurés ?\pause \hspace{.2cm} $\to$ \begin{minipage}[t]{6cm}
				s'adaptent à des formes réalistes.\pause
			\end{minipage}
			\item Difficultés :\pause
			\begin{itemize}
				\item Gradient\textbf{s} discret\textbf{s}.\pause\\ 
				$\to$ proposition de gradient discret locaux.\pause
				\item Pas d'espace du maillage \pause $\to$ Quantifier l'erreur.\pause\\
				$\to$ proposition de pas $h$\pause
			\end{itemize}
		\end{itemize}
	...
	\begin{frame}{Majoration de l'erreur}
		\only<1>{\begin{center}
				La modélisation respective de $u$ et du gradient de $u$, est définie par :
				\vspace{.5cm}
	...
	\begin{frame}{Tests numériques : Maillages}
		...
		\underline{Condition de bord :} périodique\pause $\implies$ \begin{minipage}[t]{6cm}
			même valeur en haut et en bas, à gauche et à droite.
		\end{minipage}
	\end{frame}
		...
		\begin{table}
			...
			\begin{subtable}{0.47\textwidth}
				\begin{NiceTabular}{cccc}
					...
					& & & \\
					\hline
					$\varphi=0$ & 1.996 & 1.996 & 2.008 \\
					Delaunay & 1.232 & 1.216 & 1.102 \\
					$\varphi=0.5$ & 1.034 & 0.937 & 0.621 \\
					$\varphi=0.9$ & 1.032 & 0.871 & -0.365
				\end{NiceTabular}
				\caption{Gaussienne.}
			\end{subtable}
			\begin{subtable}{0.45\textwidth}
				\begin{NiceTabular}{cccc}
					...
					Delaunay & 1.352 & 1.143 & 0.620 \\
					$\varphi=0.5$ & 0.891 & 0.857 & 0.595 \\
					$\varphi=0.9$ & 0.982 & 0.882 & -0.537
				\end{NiceTabular}
				\caption{Fonction plateau.}
			\end{subtable}
			\caption{Ordre de convergence.}
		\end{table}
		}
	\end{frame}	
	\restoregeometry
