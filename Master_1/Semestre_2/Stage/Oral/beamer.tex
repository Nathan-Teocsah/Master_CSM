...
\title[Soutenance de stage]{Consistance de gradients discrets sur des maillages non-structurés 2D}
\author{N. Hascoët}
\institute[] % (optional)
{
...
\begin{document}
	...
	\begin{frame}{Introduction}
		Simulation \pause $\to$ \begin{minipage}[t]{4cm}
			Méthode aux volumes finis\only<7-8>{\\
			$\hookrightarrow$ Gradients discrets}
			\only<9>{\\
			\alert{$\hookrightarrow$ Gradients discrets}}
		\end{minipage} \pause $\to$ utilisation d'un maillage
		
		\begin{figure}[h!]
			\includegraphics[scale=.2]{image/maillage_aile_avion.png}
			\caption{Maillage d'une aile d'avion.}
		\end{figure}\pause
		\centering
		\begin{NiceTabular}{c|c}
			Forme & Taille\\
			\hline
			\only<4->{Maillage cartésien} & \only<5-8>{Pas du maillage}\only<9>{\alert{Pas du maillage}}\\
			\only<6->{Maillage triangulaire} & \only<8->{Temps de calcul}
		\end{NiceTabular}
	\end{frame}
	...
	\section{Gradients discrets locaux pour la méthode des volumes finis}
	\begin{frame}
		\begin{center}
			Étant donné $u \in \mathcal C^2(\mathbb R^2,\mathbb R)$\pause
			...
			Méthode aux volumes finis\pause
			...
			$u_i = \dfrac1{\abs{C_i}}\displaystyle\int_{C_i} u(x)\,\mathrm dx$, \hspace{.2cm} et,\hspace{.2cm} $\nabla u_i = \dfrac1{\abs{C_i}}\displaystyle\int_{C_i} \nabla u(x)\, \mathrm dx$
		\end{center}
	\end{frame}
	
	%Slide 3
	\begin{frame}{Gradients discrets locaux pour la méthode des volumes finis}
		...
		où $M_i = I_2 - \dfrac1{\abs{C_i}}\displaystyle\sum_{j \in \mathcal N_i} \lambda_{i,j} {n_{i,j}}\otimes\left( {\bar x_{i,j}-x_{i,j}}\right).$\pause
	...
	\begin{frame}{Majoration de l'erreur}
		\only<1>{
			\begin{center}
				...
				\begin{equation}
					\dfrac1{\abs{C_i}}\displaystyle\sum_{j \in \mathcal N_i} \left[- \lambda_{i,j}R_1^{ij} + R_2^{ij} - R_3^{ij}\right] n_{ij},
				\end{equation}
			\end{center}
		}
		...
		Majorations des erreurs pour \only<2-3>{$h = \max_i \max_{j \in \mathcal N_i} d(x_i,x_j)$}\only<4>{$\textcolor{red}{h = \max_i \max_{j \in \mathcal N_i} d(x_i,x_j)}$}\only<5>{$\textcolor{blue}{h = \dfrac{\left(\max_i \max_{j \in \mathcal N_i} \mathrm{diam}(C_i)+\mathrm{diam}(C_j)\right)^3}{\min_i \abs{C_i}}}$}\only<6->{$h = \dfrac{\left(\max_i \max_{j \in \mathcal N_i} \mathrm{diam}(C_i)+\mathrm{diam}(C_j)\right)^3}{\min_i \abs{C_i}}$} :
		...
		\begin{equation}\label{eq:maj_naiv_R3}
			\abs{R_3^{i,j}} \leq \lambda_{ij}{|\!|\nabla^2 u|\!|_\infty}|\!| {\bar x_{i,j}-x_{i,j}}|\!| \frac1{\abs{C_i}}\int_{C_i}|\!| {x- x_{ij}} |\!|\dx.
		\end{equation}}}
		...
		,où $|\!|M_i^{-1}|\!| = \underset{|\!|v|\!| = 1}\sup |\!|M_i^{-1}v|\!|$.
		}
		}}
	\end{frame}
	...
	\begin{frame}{Majoration de $|\!|M_i^{-1}|\!|_2$ : prérequis}
		...
		\begin{center}
			...
			\end{tikzpicture}
			\captionof{figure}{Quantité signée $\delta_{i,j}$ représente le produit scalaire entre $\bar x_{i,j}-x_{i,j}$ et $n_{i,j}^\perp$.}
	...
	\begin{frame}{Majoration de $|\!|M_i^{-1}|\!|_2$ : maillage triangulaire}
		...
		Maillage de Delaunay $\implies$ $\mu_{\min} \geq \arcsin\left(\dfrac1{\sqrt{5}}\right)$.
	\end{frame}
	...
	\begin{frame}{Majoration de $|\!|M_i^{-1}|\!|_2$ : maillage triangulaire}
		...
		\only<8->{
			...
			\begin{center}
				$|\!|M_i^{-1}|\!|_2 \leq \sqrt{\frac32+\left(\frac{3}{2\sin^2(\mu_{\min})}\right)^2}$
			\end{center}
		}
	\end{frame}	
	...
	\begin{frame}{Majoration de $|\!|\nabla u_i - \overline{\nabla u(x_i)}|\!|_2$ : maillage triangulaire}
		...
		\begin{equation}
			|\!|\nabla u_i - \overline{\nabla u(x_i)}|\!|_2 \leq 6\sqrt{\frac32+\left(\frac{3}{2\sin^2(\mu_{\min})}\right)^2}{|\!|\nabla^2 u|\!|_\infty}\, h.
		\end{equation}
	\end{frame}
	...
	\begin{frame}{Tests numériques : Fonctions tests}
		\begin{itemize}
			\only<1,2>{\item Gaussienne : $u^{(1)}(x,y) = \exp\left(\dfrac{(x-x_c)^2+(y-y_c)^2}{2\sigma^2}\right)$\\
			\only<1>{
				\vspace{.5cm}
				\centering
	...
	\newgeometry{left=-.5cm, right=0cm, top=0.5cm, bottom=0.5cm}
	\begin{frame}{\hspace{1cm}Résultats}
		\only<1>{
			\centering
		\begin{figure}
		\begin{subfigure}{0.47\textwidth}
			\centering
			\includegraphics[scale=.42]{image/carre.png}
			\caption{Gaussienne $u^{(1)}$.}
		\end{subfigure}
		\begin{subfigure}{0.45\textwidth}
			\centering
			\includegraphics[scale=.42]{image/carre_plateau.png}
		...
		\begin{table}
			\begin{NiceTabular}{cccc}
				...
				\rowcolor{blue!15}{4}
				\Body
				\Block[C]{2-1}{Fonction} & \Block[C]{2-1}{$\mathrm L^1$} & \Block[C]{2-1}{$\mathrm L^2$} & \Block[C]{2-1}{$\mathrm L^\infty$} \\
				& & & \\
				\hline
				$u^{(1)}$ & 1.996 & 1.996 & 2.008 \\
				$u^{(2)}$ & 1.972 & 1.925 & 1.920
			\end{NiceTabular}
			\caption{Ordre de convergence pour des maillages cartésiens.}
		\end{table}}
		...
		\begin{figure}
			\begin{subfigure}{0.47\textwidth}
				\centering
				\includegraphics[scale=.42]{image/deforme_05.png}
				\caption{Gaussienne $u^{(1)}$.}
			\end{subfigure}
			\begin{subfigure}{0.45\textwidth}
				\centering
				\includegraphics[scale=.42]{image/deforme_05_plateau.png}
				\caption{Fonction plateau $u^{(2)}$.}
			\end{subfigure}
		\end{figure}
		...
		\begin{table}
			...
			\end{NiceTabular}
			\caption{Maillages cartésiens déformés de déformation $0.5$.}
		\end{table}}
		...
		\only<4>{
			\begin{figure}
				\begin{subfigure}{0.47\textwidth}
					\centering
				\includegraphics[scale=.42]{image/deforme_09.png}
				\caption{Gaussienne $u^{(1)}$.}
				\end{subfigure}
				\begin{subfigure}{0.45\textwidth}
					\centering
					\includegraphics[scale=.42]{image/deforme_09_plateau.png}
					\caption{Fonction plateau $u^{(2)}$.}
				\end{subfigure}
		\end{figure}
		...
		\only<2>{
			\begin{figure}
				\begin{subfigure}{0.47\textwidth}
					\centering
				\includegraphics[scale=.42]{image/ffm.png}
				\caption{Gaussienne $u^{(1)}$.}
				\end{subfigure}
				\begin{subfigure}{0.45\textwidth}
					\centering
					\includegraphics[scale=.42]{image/ffm_plateau.png}
					\caption{Fonction plateau $u^{(2)}$.}
				\end{subfigure}
		\end{figure}
	...
	\begin{frame}
		\begin{center}
			...
			\begin{itemize}
				\item Nouveau pas $h$ \pause $\to$ quantifie l'erreur de consistance.\pause
				\item Erreur de consistance d'ordre 1 : \pause \begin{minipage}[t]{5cm}
					pour les maillages triangulaires et quadrangulaires.
				\end{minipage}\pause
				\item Convergence confirmée : \pause maillage vérifiant les conditions énoncées.\pause
				\item Convergence mise en défaut : \pause maillage ne respectant pas les conditions.\pause
				\item Pas $h$ difficile à diminuer : \pause Nombre de cellule $\nearrow$ $\cancel\implies$ $h\searrow$.
			\end{itemize}
		\end{center}
	\end{frame}
