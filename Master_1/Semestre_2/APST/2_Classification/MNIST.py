...
dirdat=""
digits=np.loadtxt(dirdat+"mnist_test.csv", delimiter=',',skiprows=1,usecols=range(1,28*28+1))
labels1=np.loadtxt(dirdat+"mnist_test.csv", delimiter=',',skiprows=1,usecols=0)
labels=np.zeros((len(labels1),2))
for k in range(len(labels)):
  labels[k] = [k+1,labels1[k]]
...
print('Premières étiquettes: ',labels1[range(20)])
...
for k in range(k_means.n_clusters):
  counts=np.unique(labels1[cl==k],return_counts=True) # Nb d'occurences de chaque label
...
print(*(maj_lab))
err=sum(labels1!=maj_lab[cl])/len(cl)
...
conf_mat =  confusion_matrix(labels1,maj_lab[cl])
...
fig=plt.figure(3)
conf_mat =  confusion_matrix(labels1,cl)
