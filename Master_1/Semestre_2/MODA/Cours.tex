...
\begin{document}
	...
	\hspace{-.5cm}\includegraphics[scale=.4]{photo7.png}
	...
	\subsection{Linéarisation d'un EVP (\textit{Eigen Value Problem}) quadratique}
...
\subsubsection{Chaîne mono-atomique}
	...
	\begin{sol}$ $
		...
		\begin{enumerate}
			\item On identifie la période : \includegraphics[scale=.2]{photo9.png} 
	...
	\begin{exo}[applicatif] \includegraphics[scale=.5]{photo31.png}\\
		\begin{enumerate}[label=$\cdot$]
	...
	\begin{exo}
		...
		\begin{minipage}[t]{18cm}
			Puis appliquer au système suivant : \hspace{.5cm} \includegraphics[scale=.5]{photo35.png}
		\end{minipage}
	\end{exo}
	\begin{sol}$ $\\
		\begin{minipage}[t]{18cm}
		...
		\begin{minipage}{18cm}
			...
			\end{cases}$, \begin{minipage}[t]{15cm}
				cellule : \includegraphics[scale=.5]{photo36.png}\\
	...
	\begin{exo}		
		\begin{minipage}[t]{6cm}
			...
			\begin{minipage}{6cm}
				\includegraphics[scale=.5]{photo41.png}
			\end{minipage}\begin{minipage}{10cm}
			où $\mathbb D = \begin{bmatrix}
				D_L & D_i & 0 \\
				D_{iL} & D_i & D_{iR} \\
				0 & D_{Ri} & D_R
			\end{bmatrix}$
		...
		\begin{enumerate}
			\item  \begin{minipage}[t]{18cm}
				Assembler le système global traduisant l'équilibre dynamique sous la forme : $\mathbb D_y\begin{bmatrix}
				u_0 \\ P_0\\ u_1 \\ \vdots \\ u_{N-1} \\ P_{N-1} \\ u_N
			\end{bmatrix} = \begin{bmatrix}
			F_0 \\ 0 \\ \\ \vdots \\ \\ 0 \\ f_N
			\end{bmatrix}$
			\end{minipage}
			
			\item Réécrire en fonction de la matrice $\overset\sim{\mathbb D}$ condensée (à expliciter)
			
			\item Décomposer les C.L. en ondes de Bloch
		\end{enumerate}
	\end{exo}
	\begin{sol}
		$ $
		\begin{enumerate}
			...
			\item \begin{minipage}[t]{18.5cm}
				Pour une cellule $(n)$ l'équilibre local s'écrit :\\
				\includegraphics[scale=.8]{photo45.png}\\
				...
				F_L \\ 0 \\ F_R
				\end{bmatrix} & 
				\iff  \begin{bNiceMatrix}[margin=1em]
				D_L & 0 & D_i \\
				0 & D_R & D_{Ri} \\
				D_{iL} & D_{iR} & D_i
				\CodeAfter
				\SubMatrix[{1-1}{2-2}]
				\SubMatrix[{3-3}{3-3}]
				\end{bNiceMatrix}\begin{bNiceMatrix}[margin=1em]
				u_L \\
				u_R \\
				p
				\CodeAfter 
				...
				\end{bmatrix}\\
				&\iff \Big[
					\underbrace{D_b-D_{bi}D_i^{-1}D_{ib}}_{\rouge{\overset\sim D_b}}\Big]u_b = f_b
				\end{aligned}$\\
				$\begin{aligned}
					\overset\sim D &= \underbrace{\begin{bmatrix}
						D_L & 0 \\
						0 & D_R
					\end{bmatrix}}_{D_b} - \underbrace{\begin{bmatrix}
					D_{Li}\\
					D_{Ri}
				\end{bmatrix}}_{D_{bi}}D_i^{-1}\underbrace{\begin{bmatrix}
				D_{iL} & D_{iR}
			\end{bmatrix}}_{D_{ib}} = \begin{bmatrix}
			D_L-D_{Li}D_i^{-1}D_{iL} & -D_{Li}D_i^{-1}D_{iR} \\
			-D_{Ri}D_i^{-1}D_{iL} & D_R-D_{Ri}D_i^{-1}D_{iR}
		\end{bmatrix}\\
		&=\begin{bmatrix}
			\overset\sim D_L & \overset\sim D_{LR} \\
		...
		\begin{minipage}{18.5cm}
			Le système assemblé devient alors : $\begin{bmatrix}
			\overset\sim D_L & \overset\sim D_{LR} \\
			\overset\sim D_{RL} & \overset\sim D_R + \overset\sim D_L & \cdots \\
			& & \ddots \\
			& & \cdots & \overset\sim D_R + \overset\sim D_L & \overset\sim D_{LR} \\
			& & & \overset\sim D_{RL} & \overset\sim D_R
		\end{bmatrix}\begin{bmatrix}
		...
		\item \begin{minipage}[t]{18.5cm}
			$\begin{cases}
			\text{en }n=0 : \overset\sim D_L u_0 + \overset\sim D_{LR} u_1 = F_0 \\
			\text{en }n=N : u_N = V_N
		\end{cases}$\\
		Décomposition en ondes de Bloch : $\forall n \in \N, u_n = \Phi^+\Lambda^n a^+ + \Phi^-\Lambda^{N-n}a^-$\\
		Ce qui donne : $\begin{cases}
			\overset\sim D_L\left(\Phi^+a^+ + \Phi^- \Lambda^Na^-\right)+\overset\sim D_{RL}\left(\Phi^+ \Lambda a^+ + \Phi^-\Lambda^{N-1}a^-\right) = F_0 \\
			\Phi^+\Lambda^Na^+ + \Phi^-a^- = V_N
		\end{cases}$\\
		qui s'écrit : $\underbrace{\begin{bmatrix}
				\overset\sim D_L\Phi^+ + \overset\sim D_{LR}\Phi^+\Lambda & \overset\sim D_L\Phi^-\Lambda^{N-1}\\
				\Phi^+ \Lambda^N&  \Phi^-
	...
	\end{bmatrix}= H^{-1}\begin{bmatrix}
	F_0 \\ V_N
	\end{bmatrix}$, ce qui donne $\forall n \in [\![0,N]\!], u_n = \Phi^+\Lambda^na^+ + \Phi^- \Lambda^{N-n}a^-$
		\end{minipage}
		\end{enumerate}
	\end{sol}
	...
	\begin{exo}
		\begin{minipage}{6cm}
			\includegraphics[scale=.5]{photo42.png}
		\end{minipage}\\
		\begin{enumerate}
			\item Assembler le système global avec $S$
			
			\item  Assembler avec $\overset\sim S$ (condensation de $p$ sur $(u_i,v_i)$).
			
			\item  Ecrire les conditions d'équilibre des forces aux interfaces de $S$.
			
			\item Décomposer en ondes
			
			\item \begin{minipage}[t]{17cm}
				 En supposant une onde incidente d'amplitude $1$ (i.e. $a^+ = \vec1$), déterminer le système à résoudre pour connaître les ondes réfléchie ($R=a^-$) et transmise ($T = b^+$)
			\end{minipage}
		\end{enumerate}
	\end{exo}
	\begin{sol}
		$ $
		\begin{enumerate}
			\item \begin{minipage}{18.5cm}
			$\begin{bNiceMatrix}[margin=0.1em]
				\Ddots \\
				& D_R+D_L & D_{LR} \\
				 &  D_{RL} & D_R+S_L & S_{Li} & 0\\
				 & 		   & S_{iL} & S_i & S_{iR} \\
				 &  	   & 	0	& S_{Ri} & S_R+D_L & D_{LR} \\
				 &		   &	    &        & D_{RL} & D_R+D_L\\
				 & & & & & & \Ddots
			\end{bNiceMatrix}\begin{bNiceMatrix}
			\Vdots \\ u_0 \\ u_1 \\ p \\ v_0 \\ v_1 \\ \Vdots
			\end{bNiceMatrix} = \begin{bNiceMatrix}
			\Vdots \\ \\ \\ 0 \\ \Vdots \\ \\ \\
			\end{bNiceMatrix}$
			\end{minipage}
			...
			\item \begin{minipage}[t]{18.5cm}
				$\overset\sim S_b = \rouge{S_b}-\bleu{S_{bi}}S_i^{-1}S_{ib}$, où $b = \begin{bmatrix}
				L \\ R
			\end{bmatrix}$ \\
			$S = \begin{bNiceMatrix}[margin=1em]
				\rouge{S_L} & \rouge0 & \bleu{S_{Li}}\\
				\rouge0 & \rouge{S_R} & \bleu{S_{Ri}} \\
				S_{iL} & S_{iR} & S_i\\
				\CodeAfter
				\SubMatrix[{1-1}{2-2}]
				\SubMatrix[{1-3}{2-3}]
			\end{bNiceMatrix}$, $\begin{aligned}[t]
			\overset\sim S &= \begin{bmatrix}
			S_L & 0 \\
			0 & S_R
			\end{bmatrix} - \begin{bmatrix}
			S_{Li} \\
			S_{Ri}
			\end{bmatrix}S_i^{-1}\begin{bmatrix}
			S_{iL} & S_{iR}
			\end{bmatrix}\\
			&= \begin{bmatrix}
				S_L-S_{iL}S_i^{-1}S_{iL} & -S_{Li}S_i^{-1}S_{iR} \\
				-S_{Ri}S_i^{-1}S_{iR} & S_R - S_{Ri}S_i^{-1}S_{iR}
			\end{bmatrix}\\
			&=\begin{bmatrix}
				\overset\sim S_L & \overset\sim S_{LR} \\
				\overset\sim S_{RL} & \overset\sim S_R
			\end{bmatrix}
		\end{aligned}$\\
		On peut alors représenter le système comme :\\
		\includegraphics[scale=.5]{photo43.png}\\
		On assemble alors : $\begin{bNiceMatrix}[margin=0.1em]
			\Ddots \\
			& D_R+D_L & D_{LR} \\
			&  D_{RL} & D_R+\overset\sim S_L &\overset\sim S_{LR} \\
			&  	   	&\overset\sim S_{RL} &\overset\sim S_R+D_L & D_{LR} \\
			&		   &	    &        & D_{RL} & D_R+D_L\\
			& & & & & & \Ddots
		\end{bNiceMatrix}\begin{bNiceMatrix}
			\Vdots \\ u_0 \\ u_1 \\ v_0 \\ v_1 \\ \Vdots
		\end{bNiceMatrix} = 0$
			\end{minipage}
			
			\item \begin{minipage}[t]{15cm}
				On développe les lignes correspondantes pour l'équation en $u_1$ et $v_0$ : 
				
				\[
				\begin{cases}
					D_{RL} u_0 + (D_R+\overset\sim S_L)u_1 + \overset\sim S_{LR} v_0 = 0 & (1) \\
					\overset\sim S_{RL} u_1 + (\overset\sim S_R + D_L)v_0 + D_{LR} v_1 = 0 & (2)
				\end{cases}
				\]
			\end{minipage}
			
			\item $\forall n, \begin{aligned}[t]
				u_n = \Phi^+\Lambda^n a^+ + \Phi^- \Lambda^{\overset{=1}{N-n}}a^- \\
				v_n = \Phi^+ \Lambda^+ b^+ + \Phi^- \Lambda^{\overset{=1}{M-n}}b^-
			\end{aligned}$\\
			\hspace*{-1cm}$\begin{cases}
				\left[D_{RL}\Phi^+ + (D_R+\overset\sim S_L)\Phi^+\Lambda\right]a^- + \overset\sim S_{LR}\Phi^+b^+ + \overset\sim S_{LR} \Phi^-\Lambda b^- = 0 & (1)\\\\
				\overset\sim S_{RL}\Phi^+ \Lambda a^+ + \overset\sim S_{RL}\Phi^- a^- + \left[(\overset\sim S_R +D_L)\Phi^+ +D_{LR}\Phi^+ \Lambda\right]b^+ + \left[(\overset\sim S_R + D_L)\Phi^-\Lambda + D_{LR}\Phi^-\right]b^- = 0 & (2)
			\end{cases}$\newpage
			$\to$ qui s'écrit : $ H\begin{bmatrix}
				a^+ \\ a^- \\ b^+ \\ b^-
			\end{bmatrix} = 0$
			
			\item  \begin{minipage}[t]{15cm}
				On suppose connue une onde individuelle ($a^+ = 1$)\\
			Calculons les coefficients de Réflexion et Transmission\\
			\includegraphics[scale=.5]{photo44.png}\\
			Le système devient :
			\[
			\begin{aligned}
				ca^+ = - dR-\overset\sim S_{LR}\Phi^+T \\
				\overset\sim S_{RL}\Phi^+\Lambda a^+ = - \overset\sim S_{LR}\Phi^- R-f T\\
				\begin{bmatrix}
					c \\ \overset\sim S_{RL}\Phi^+\Lambda
				\end{bmatrix}a^+ = \begin{bmatrix}
				-d & -\overset\sim S_{LR}\Phi^+ \\
				-\overset\sim S_{RL}\Phi^- & - f
				\end{bmatrix}\begin{bmatrix}
				R \\ T
				\end{bmatrix}
			\end{aligned}
			\]
			Les coefficients de Réflexion et de Transmission des ondes de Bloch au travers d'un élément de couplage $S$ s'obtient par : $\begin{bmatrix}
				R \\ T
			\end{bmatrix}\begin{bmatrix}
			d & \overset\sim S_{LR}\Phi^+ \\
			\overset\sim S_R\Phi^- & -f
			\end{bmatrix}\begin{bmatrix}
			c \\ \overset\sim S_{RL}\Phi^+ \Lambda
			\end{bmatrix}a^+$
			\end{minipage}
	...
	\begin{exo}
		Structure $\infty$ avec force interne.\\
		\hspace*{3cm}\includegraphics[scale=.5]{photo46.png}
		
		\begin{enumerate}
			\item Écrire le système décrivant l'équilibre dynamique global.
			
			\item Décomposer en ondes les déplacements
			
			\item \begin{minipage}[t]{17cm}
				Résoudre et en déduire la solution $\forall n \in \N$ , $u_n$ et $D =\begin{bmatrix}
				D_L & D_{LR} \\
				D_{RL} & D_R
			\end{bmatrix}$
			\end{minipage}
	...
	\begin{sol}
		...
		\begin{enumerate}
			...
			\item \begin{minipage}{6cm}
				\includegraphics[scale=.7]{photo47.png}
			\end{minipage}\\
			
			$\begin{cases}
				u_n = \Phi^- \Lambda^n a^- \\
				v_n = \Phi^+ \Lambda^n b^+
			\end{cases}$ \begin{minipage}{13cm}
			\underline{\textbf{Remarque :}}\\
			on aurait pu prendre $n$ classique, $u_n = \Phi^-\Lambda^{N-n}a^-$ puis $u_{n'}=\Phi^-\Lambda^{-n'}$ avec $N=0$ puis $n=-n'$.
			\end{minipage}\\
			
			$\begin{aligned}
				&\begin{cases}
				D_{RL} u_1 + (D_L+D_R)u_0 + D_{LR} v_1 = F_0\\
				u_0 = v_0
			\end{cases}\\
			\iff & \begin{cases}
				\left[D_{RL}\Phi^-\Lambda+(D_L+D_R)\Phi^-\right]a^- +D_{LR}\Phi^+\Lambda b^+= F_0 \\
				\Phi^- a^- = \Phi^+b^+
			\end{cases}\\
			\iff& \begin{cases}
				b^+ = (\Phi^+)^{-1}\Phi^-a^- \\
				\underbrace{\left[D_{RL}\Phi^-\Lambda+(D_L+D_R)\Phi^- + D_{LR}\Phi^+ \Lambda(\Phi^+)^{-1}\Phi^-\right]}_{H}a^- = F_0
			\end{cases}\\
			\iff & \begin{cases}
				a^- = H^{-1} F_0\\
				b^+ = (\Phi^+)^{-1}\Phi^- H^{-1}F_0
		...
		\item 	$\forall n \in \N, \begin{aligned}[t]
				u_n &= \Phi^-\Lambda^n H^{-1}F_0 \\
				v_n &= \Phi^+ \Lambda^n(\Phi^+)^{-1}\Phi^-H^{-1}F_0
			\end{aligned}$
		\end{enumerate}
	\end{sol}
	
	\begin{exo}
		Système périodique \underline{fini} avec force interne :\\
		 	\includegraphics[scale=.6]{photo48.png}
		 
		 \begin{enumerate}
		 	\item $D$ condensé, assembler le système global
		 	
		 	\item  Écrire les 4 conditions aux limites (efforts et/ou déplacements)
		 	
		 	\item Décomposer en ondes pour déterminer la matrice $H$ telle que :
		 	\[H\begin{bmatrix}
		 		a^+ \\ a^- \\ b^+ \\ b^-
		 	\end{bmatrix} = \begin{bmatrix}
		 	U_0 \\ 0 \\ F_M \\ V_N
	...
	\begin{sol}$ $
		\begin{enumerate}
			...
			\item $\begin{cases}
				...
				v_N = V_n \\
				u_M = v_0 \\
				 D_{RL} v_{M-1} + (D_L+D_R)v_M + D_{LR} v_1 = F_M
			\end{cases}$
			
			\item $u_m = \Phi^+ \Lambda^ma^++\Phi^-\Lambda^{M-m}a^-$\\
			$v_n = \Phi^+ \Lambda^nb^++\Phi^-\Lambda^{M-m}b^-$\\
			$\begin{cases}
				\Phi^+ a^+ +  \Phi^-\Lambda^Ma^- = U_0 \\
				\Phi^+ \Lambda^Nb^+ + \Phi^- b^- = V_N \\
				\Phi^+\Lambda^Ma^+ + \Phi^-a^- = \Phi^+b^+ + \Phi^-\Lambda^Nb^- \\
				D_{RL}(\Phi^+ \Lambda^{M-1}a^+ + \Phi^- \Lambda a^-)+(D_L+D_R)(\Phi^+\Lambda^Ma^++\Phi^- a^-)+D_{LR}(\Phi^+\Lambda b^++\Phi^-\Lambda^{N-1}b)=F_M
			\end{cases}$\\
			
			$H = \begin{bmatrix}
				\Phi^+ & \Phi^-\Lambda^M & 0 & 0 \\
				\Phi^+\Lambda^M & \Phi^- & -\Phi^+ & - \Phi^-\Lambda^N \\
				D_{RL}\Phi^+\Lambda^{M-1}+(D_L+D_R)\Phi^+\Lambda^M & D_{RL}\Phi^-\Lambda + (D_L+D_R)\Phi^- & D_{LR}\Phi^+ \Lambda & D_{LR}\Phi^-\Lambda^{N-1} \\
				0 & 0& \Phi^+\Lambda^N & \Phi^-
			\end{bmatrix}$
	...
	\begin{exo}\begin{minipage}[t]{18cm}
			Deux guides d'ondes périodiques - diffusion des ondes
		\end{minipage}
		\includegraphics[scale=.5]{photo49.png}\\
		
		On note $\begin{cases}
			(\Phi^\pm,\Lambda) & \text{les ondes dans } \mathbb{D}^{(1)} \\
			(\Psi^\pm,\Delta) & \text{les dondes dans }\mathbb{D}^{(2)}
		\end{cases}$\\
		On considère une onde de Bloch incidente, d'amplitude $a^+$.\\
		\underline{Question :} \begin{minipage}[t]{13cm}
			Ecrire le système permettant de calculer les amplitudes transmises ($T$) et réfléchies ($R$) à l'interface.
		\end{minipage}
	\end{exo}	
	\begin{sol}$ $\\
		\begin{minipage}[t]{18.5cm}
			On commence par assembler le système d'équation : $\begin{bNiceMatrix}[margin=1em]
				\Ddots \\
				 & D_R^1 + D_L^1 & D_{LR}^1 \\
				  & D_{RL} ^1 & D_R^1 + D_L^2 & D_{LR}^2 \\
				  &			  & D_{RL}^2      & D_R^2 + D_L^2 \\
				  &			  &				  &				& \Ddots
			\end{bNiceMatrix}$\\
			La continuité des déplacements et l'équilibre des forces à la jonction s'écrivent :\\
			$\begin{cases}
				u_1 = v_0 \\
				D_{RL}^1u_0 + (D_R^1+D_L^2)u_1 + D_{LR}^2 v_ 1 = 0
			\end{cases}$\\
			
			La décomposition en ondes de Bloch donne :\\
			$\begin{cases}
				u_n = \Phi^+ \Lambda^na^++\Phi^-\Lambda^{1-n}a^- & \text{pour $N= 1$ (convention arbitraire)}\\v_n = \Psi^+\Delta^ + + \Psi^-\Delta^{-n}b^-
			\end{cases}$ \\
			La réflexion $a^- =: R$\\
			La transmission $b^+ =: T$\\
			2 Équations : \\
			$\begin{cases}
				\Phi^+ \Lambda a^+ + \Phi^-R=\Psi^+ T\\
				(\underbrace{D_{RL}\Phi^+(D_L+D_R)\Phi^+\Lambda}_{\alpha^+})a^+ + (\underbrace{D_{RL}\Phi^-\Lambda+(D_L+D_R)\Phi^-}_{\alpha^-})R+D_{LR}\Psi^+\Delta T = 0
			\end{cases}$\\
			
			$\begin{bmatrix}
				\Phi^- &  - \Psi^+ \\
				\alpha^- & D_{LR}\Psi^+\Delta
			\end{bmatrix}\begin{bmatrix}
			R \\ T
			\end{bmatrix} = \begin{bmatrix}
			-\Phi^+ \Lambda \\
			...
			Donc on obtient les coefficients de réflexion et Transmission des ondes via :
