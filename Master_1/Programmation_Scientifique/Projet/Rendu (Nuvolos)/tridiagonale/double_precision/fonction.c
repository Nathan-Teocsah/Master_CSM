...
void initiate_A_b(Matrix_double  *A,  Matrix_double  *D, Matrix_double  *DL, Matrix_double  *DU,  Matrix_double *X, Matrix_double  *b, Matrix_double *Res, double h)
{
  ...
  Matrix_double* S[7] = {A, D, DL, DU, b, X, Res};
  int lign,col;
  for (int i=0; i < 7; i++){
    lign = S[i]->nrow;
    col = S[i]->ncol;
    S[i]->mat = malloc(lign*sizeof(*S[i]->mat));
    S[i]->p = calloc(lign*col,sizeof(*S[i]->p));
    for (int j = 0; j<col;j++) 
    {
      S[i]->mat[j] = S[i]->p + j*col; 
    }
  }
...
void produit(Matrix_double A, Matrix_double X, Matrix_double  *Res)
{
  int n = A.nrow;
