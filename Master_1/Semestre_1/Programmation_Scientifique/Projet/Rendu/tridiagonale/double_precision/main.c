...
//=================== AFFICHAGE DES SPÉCIFICATIONS DE l'ALGORITHME  ==================================
  ...
  FILE *fp = fopen("resultat_tri_double.txt", "w");
  fprintf(fp,"n");
  ...
  Matrix_double A, D, DL, DU, b, X, Res; 
  ...
  for (int n = n_min; n<=n_max; n += n_pas)
  {
    ...
    initiate_A_b(&A,&D,&DL,&DU,&b,&X,&Res,h);
  
    
  //-------------------   RESOLUTION De AX=B  ----------------------
  
    int INFO;
    dgtsv_(&n, &b.ncol, DL.p, D.p, DU.p, X.p, &n, &INFO);
